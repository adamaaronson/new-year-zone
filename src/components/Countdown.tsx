...
function getSeconds(timeInMilliseconds: number) {
  ...
  return Math.floor(
    (timeInMilliseconds / MILLISECONDS_PER_SECOND + 1) % SECONDS_PER_MINUTE
  )
    .toString()
    .padStart(2, "0");
}
...
function getMinutes(timeInMilliseconds: number) {
  ...
  return Math.floor(
    ((timeInMilliseconds + MILLISECONDS_PER_SECOND) / MILLISECONDS_PER_MINUTE) %
      MINUTES_PER_HOUR
  )
    .toString()
    .padStart(2, "0");
...
function getHours(timeInMilliseconds: number) {
  ...
  return Math.floor(
    ((timeInMilliseconds + MILLISECONDS_PER_SECOND) / MILLISECONDS_PER_HOUR) %
      HOURS_PER_DAY
  )
    .toString()
    .padStart(2, "0");
...
export function getDays(timeInMilliseconds: number) {
  ...
  return Math.floor(
    (timeInMilliseconds + MILLISECONDS_PER_SECOND) / MILLISECONDS_PER_DAY
  );
}
...
export function getNewYearInTimeZone(newYear: number, timeZone: TimeZone) {
  ...
  timeZoneNewYearDate.setMinutes(
    timeZoneNewYearDate.getMinutes() + differenceMinutes
  );
...
export function getTimestampDescription(timestamp: number, timeZone: TimeZone) {
  if (isLocalTimeZone(timeZone)) {
    return (
      <>
        <FontAwesomeIcon icon={faMapMarkerAlt} /> You are here
      </>
    );
  }

  const newYearLocalTime = new Date(timestamp).toLocaleTimeString([], {
    hour: "numeric",
    minute: "numeric",
  });
  const newYearLocalDate = new Date(timestamp).toLocaleDateString([], {
    month: "short",
    day: "numeric",
  });
...
export function Countdown({ untilTime, globalTime, onCountdownEnd }: Props) {
  ...
  return <div className="countdown">{getTimeString(time)}</div>;
}
